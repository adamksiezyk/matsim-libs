package org.matsim.codeexamples.psoSignals;
 ...
 * @author tthunig
 */
public class RunPsoSignalsExample {

	private static final Logger log = LogManager.getLogger(RunPsoSignalsExample.class);

	public static void main(String[] args) {
		String configFileName = "./contribs/signals/examples/pso/config.xml";
		if (args != null && args.length != 0) {
			log.info("Your config file " + args[0] + " will be loaded and run with signals and otfvis-visualization.");
			configFileName = args[0];
		}
		run(configFileName, "output/runPsoSignalsExampleOutput/", false); // The run method is extracted so that a test can operate on it.
	}
